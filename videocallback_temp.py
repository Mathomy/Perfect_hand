...
class LoggingVideoCallback(BaseCallback):
    ...
    def __init__(self, env, save_dir="logs", video_freq=10, save_success_videos=True, verbose=1, frame_skip=1):
        ...
        os.makedirs(f"{save_dir}/successful_episodes/videos", exist_ok=True)
        os.makedirs(f"{save_dir}/successful_episodes/trajectories", exist_ok=True)
    ...
    def _on_step(self) -> bool:
        ...
        if done:
            ...
            is_success = info.get('success', False) or info.get('dist_fingers', 1.0) < 0.05
            ...
            if is_success and len(self.frames) <20:
                success_video = f"{self.save_dir}/successful_episodes/videos/episode_{self.episode_count:04d}.mp4"
                success_traj = f"{self.save_dir}/successful_episodes/trajectories/trajectory_{self.episode_count:04d}.pkl"
                imageio.mimsave(success_video, self.frames, fps=5)
                with open(success_traj, "wb") as f:
                    pickle.dump(self.current_episode, f)

            # --- Capture final frame BEFORE reset ---
            
            last_frame = self.env.envs[0].get_frame()
            ...
            # --- Capture first frame of the NEW episode ---
